...
class CloudAPI {
    ...
    private func writeToObsidian(note: String, date: Date) async {
        let vaultPath = (configManager.config.obsidianVaultPath as NSString).expandingTildeInPath
        let dateString = formatDate(date)
        var filePath = "\(vaultPath)/Daily Notes/\(dateString).md"
        
        // Create directory if needed
        let directory = (filePath as NSString).deletingLastPathComponent
        try? FileManager.default.createDirectory(atPath: directory, withIntermediateDirectories: true)
        
        // Non-destructive write: If file exists, create a new separate file
        ...
        if fileManager.fileExists(atPath: filePath) {
            ...
            // New format: YYYY-MM-DD (Generated HH-mm-ss).md
            filePath = "\(vaultPath)/Daily Notes/\(dateString) (Generated \(timeString)).md"
        }
