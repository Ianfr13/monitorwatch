...
3. **FIX OCR ERRORS**: WhitsApp→WhatsApp, Antigr4vity→Antigravity, Wi$pr→Whisper, Xrodg→Xcode, Tennlno1→Terminal
...
Você é um assistente pessoal integrado ao Obsidian.
Crie uma nota diária completa a partir dos resumos por hora abaixo.
...
Gere apenas o conteúdo Markdown abaixo, sem conversas.
...
## Reuniões & Insights
> [!quote] Conversas
> <Resumo de reuniões e conversas se houver>
...
3. **CORRIGIR ERROS DE OCR**: WhitsApp→WhatsApp, Antigr4vity→Antigravity, Wi$pr→Whisper, Xrodg→Xcode, Tennlno1→Terminal
...
7. **IDIOMA**: Saída estritamente em Português do Brasil.
`;
}
...
Você é um assistente pessoal integrado ao Obsidian.
Analise os logs e crie uma nota diária direta, profissional e limpa.
...
## Resumo
<Parágrafo narrativo sobre o dia. Use tags (#projeto, #tech) e links ([[Link]]) naturalmente no meio das frases.>
...
4. **IDIOMA**: Saída estritamente em Português do Brasil.
`;
}
...
function buildMeetingPrompt(activities: Activity[], transcripts: Transcript[], context: string, date: string, language: string): string {
    let transcriptsSummary = language === 'pt' ? 'Nenhuma transcrição disponível.' : 'No transcripts available.';
    if (transcripts.length > 0) {
        transcriptsSummary = '';
...
Você é um analista de dados pessoais especializado integrado ao Obsidian.
Você recebe um log das últimas 2 horas de atividade do usuário (Apps e Áudio).
Seu objetivo é identificar a **Sessão Lógica Atual** e gerar uma nota Markdown de alta qualidade.
...
1.  **SEGMENTAÇÃO**: Analise os logs de trás para frente. Identifique quando o contexto atual começou.
    *   Se palavras como "Reunião", "Daily", "Alinhamento" aparecem no áudio -> É uma **Reunião**.
    *   Se uso intenso de VS Code, Terminal, Xcode -> É uma **Sessão de Código**.
    *   Se uso de Browser, YouTube, Leitura -> É uma **Sessão de Pesquisa/Estudo**.

2.  **EXTRAÇÃO**: Extraia *apenas* informações relevantes para esta sessão específica.

3.  **GERAÇÃO**: Gere uma nota Markdown específica baseada no tipo detectado.

---

## FORMATOS DE SAÍDA

### MODO A: REUNIÃO (Padrão se conversa detectada)
\`\`\`markdown
---
date: ${new Date(date).toISOString().split('T')[0]}
type: meeting
...
### MODO B: SESSÃO DE CÓDIGO/TRABALHO (Se principalmente uso técnico)
\`\`\`markdown
---
date: ${new Date(date).toISOString().split('T')[0]}
type: dev-log
...
# <Título baseado no Projeto/Atividade>

## Contexto
<O que estava sendo trabalhado>

## Alterações & Detalhes
- **<App/Arquivo>**: <Detalhes baseados em Títulos/OCR>

## Insights/Decisões
> [!tip] Nota Técnica
> <Decisão técnica ou aprendizado>
\`\`\`

---

## REGRAS CRÍTICAS
1.  **Auto-Detectar Início**: Não resuma as 2 horas inteiras. Encontre o início lógico.
2.  **Português-Brasil**: Saída estritamente em PT-BR.
3.  **Sem Enrolação**: Seja direto e conciso.
4.  **Sem Emojis**: Zero emojis na saída.
